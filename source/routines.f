...
c     If there are no free variables or B=I, then
...
c======================== The end of bmv ===============================
     ...
     +                  v, nint, sg, yg, iprint, sbgnrm, info)
...
* Citation: Kopp, J. Efficient numerical diagonalozation of hermitian 3x3 
* matrices, J. Mod. Phys. C. 19 (2008) 523-548
* ----------------------------------------------------------------------------
...
*       Test for convergence
        ...
        DO 50 X = 1, N
          DO 51 Y = X+1, N
            SO = SO + ABS(DREAL(A(X, Y))) + ABS(DIMAG(A(X, Y)))
   51     CONTINUE
   50   CONTINUE
        IF (SO .EQ. 0.0D0) THEN
          RETURN
...
*       Do sweep
        DO 60 X = 1, N
          DO 61 Y = X+1, N
            G = 100.0D0 * ( ABS(DREAL(A(X, Y))) + ABS(DIMAG(A(X, Y))) )
            IF ( I .GT. 4 .AND. ABS(W(X)) + G .EQ. ABS(W(X))
     $                    .AND. ABS(W(Y)) + G .EQ. ABS(W(Y)) ) THEN
              A(X, Y) = 0.0D0
            ELSE IF (ABS(DREAL(A(X, Y)))
     $                + ABS(DIMAG(A(X,Y))) .GT. THRESH) THEN
...
*             Apply Jacobi transformation
              ...
              DO 70 R = 1, X-1
                T       = A(R, X)
                A(R, X) = C * T - DCONJG(S) * A(R, Y)
                A(R, Y) = S * T + C * A(R, Y)
   70         CONTINUE
              DO 80, R = X+1, Y-1
                T       = A(X, R)
                A(X, R) = C * T - S * DCONJG(A(R, Y))
                A(R, Y) = S * DCONJG(T) + C * A(R, Y)
   80         CONTINUE
              DO 90, R = Y+1, N
                T       = A(X, R)
                A(X, R) = C * T - S * A(Y, R)
                A(Y, R) = DCONJG(S) * T + C * A(Y, R)
   90         CONTINUE
...
c====================== The end of SQRABS ===============================
          ...
          SUBROUTINE moisort (inertia, energy, nclust, noff, nmut, temp)

          IMPLICIT NONE

c         .. Input ..
          DOUBLE PRECISION inertia(3*(nclust+noff+nmut)) !Array containing relative moment of inertia
          DOUBLE PRECISION energy(nclust+noff+nmut)   !Array containing relative moment of inertia
          INTEGER          nclust      !Number of initial cluster
          INTEGER          noff        !Number of offsprings
...
c         .. Arguments ..
          INTEGER          ntot
          INTEGER          i, j
          DOUBLE PRECISION B(nclust+noff+nmut)
          DOUBLE PRECISION C(nclust+noff+nmut)

c         .. Set values ..
          ntot = nclust + nmut + noff
          temp = 0.d0
...
!      write(*,*) 'nmut', nmut
      ...
      do 10 i = ntot+1, ntot+nclust
          temp = 0.d0
c         write(*,*) 'i', i
          temp = energy(i-ntot)
        do 20 j = ntot+nclust+1, 2*ntot
c              write(*,*) 'j', j
              B(j-ntot-nclust) = inertia(i)/inertia(j)
c              write(*,*) 'B', B
              C(j-ntot-nclust) = inertia(i+ntot)/inertia(j+ntot)
c              write(*,*) 'C', C
              if (abs((B(j-ntot-nclust)-1)*100) .LE. 1 .AND.
     &        (abs(C(j-ntot-nclust)-1)*100) .LE. 1) then
c              energy(j-ntot) = 0.d0     alt
c              print * , 'Index'         alt
c              write(*,*) 'temp', temp   alt
                if (temp .LE. energy(j-ntot)) then
                    energy(j-ntot) = 0.d0
                    print *, 'Index', i-ntot, j-ntot-nclust
                else
                    print *, 'Index2', i-ntot, j-ntot-nclust
                   temp = energy(j-ntot)
                   energy(i-ntot) = 0.d0                
                end if 
              end if
...
c End subroutine moisort

      END SUBROUTINE

c====================== The end of moisort ===============================
